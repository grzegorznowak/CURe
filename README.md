...
Code-debt analysis runs for every review using isolated Codex CLI executions and reports only strictly grounded findings against recognized source/configuration files: source files plus TOML/JSON manifests and explicit ecosystem lockfiles (prose-only files such as `README.md` are excluded). Multipass reviews run metric clusters concurrently using the configured multipass worker bound and deterministically append `code-debt.md` to the completed review exactly once. Single-stage reviews do the same after the main review. Artifacts bind the reviewed HEAD and a canonical analysis-config digest, so resume/follow-up/config changes rerun rather than reuse stale analysis. `CURE_CODE_DEBT_PRESET`, `CURE_CODE_DEBT_MODEL`, `CURE_CODE_DEBT_MAX_TOKEN_BUDGET`, `CURE_CODE_DEBT_TIMEOUT`, and `CURE_CODE_DEBT_REPORT_OUTPUT` override the corresponding fields. Debt grounding is intentionally fixed to `strict`.

Debt workers use the same proven Codex runtime policy as ordinary multipass step workers, including the review session's writable scratch/add-directories and approval/sandbox semantics. Each worker remains isolated in the pre-staged per-session checkout and a dedicated process registry. This is an explicit decision (PR #42, review cycle B8): OS-enforced `--sandbox read-only`/`workspace-write` modes depend on bubblewrap constructing user/network namespaces, which fails on many supported hosts (`bwrap: setting up uid map: Permission denied`, `loopback: Failed RTM_NEWADDR`); when that happens every worker tool call fails and the stage silently produces an empty, misleading report. The pipeline therefore relies on the same runtime as the review steps themselves (pre-staged disposable checkout, dedicated debt process registry, bounded generation), with safety enforced by the dedicated prompt: workers must inspect statically, must not execute repository-controlled tests/builds/hooks, and must not modify the repository; writable paths are for CURe session artifacts and scratch. Do not reintroduce a sandbox mode for the debt worker without first proving bubblewrap works on the target host (e.g. `codex exec` succeeds at a read command under that sandbox mode). CURe additionally starts debt executions with `--skip-git-repo-check`, avoiding the adapter's trust-directory whole-command retry, and passes `rollout_budget.limit_tokens` on every bounded attempt so generation and retry allocations cannot exceed the configured budget.
