...
class SubsequentReviewRuntimeMemoryTests(SubsequentReviewTestCase):
    ...
    def test_post_review_preserves_disabled_review_memory_store_override(self) -> None:
        from cure_subsequent_review.contracts import ModuleStatus
        from cure_subsequent_review.memory_store import ReviewMemoryStore
        from cure_subsequent_review.runtime import update_review_memory_after_review
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(record.module.value, "review_memory_store")
            self.assertEqual(record.status, ModuleStatus.DISABLED)
            self.assertIn("module_override_disabled", record.reasons)
            self.assertIsNone(record.artifact_path)
            self.assertFalse(store.path.exists())
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest_record = manifest["modules"]["review_memory_store"]
            self.assertEqual(manifest_record["status"], "disabled")
            self.assertEqual(manifest_record["reasons"], ["module_override_disabled"])
            self.assertNotIn("artifact_path", manifest_record)
    ...
    def test_intake_completion_memory_update_does_not_require_review_publication(self) -> None:
        from cure_subsequent_review.memory_store import ReviewMemoryStore
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            record = update_review_memory_after_intake(
                artifact_dir=artifact_dir,
                memory_store=store,
                current_head="abc123",
            ...
            payload = json.loads(store.path.read_text(encoding="utf-8"))
            self.assertEqual(payload["findings"]["G-0001"]["run_provenance"]["stage"], "intake_complete")
