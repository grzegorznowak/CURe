...
class InstallAndDoctorTests(unittest.TestCase):
    ...
    def test_readme_documents_uv_tool_install_flow(self) -> None:
        ...
        self.assertIn("Before using CURe, refresh `<CURE_SOURCE>` with `git -C <CURE_SOURCE> pull --ff-only`", skill)
        self.assertIn("If `cure` is already installed and working, use the refreshed checkout.", skill)
    ...
    def test_skill_documents_proactive_secret_and_config_remediation(self) -> None:
        ...
        self.assertIn("operator owns provider choice", skill)
        self.assertIn("Do not enumerate all ChunkHound embedding providers.", skill)
        self.assertIn("Do not suggest or install Ollama unless the operator explicitly asks for a local provider path.", skill)
        self.assertIn("point me at that config now", skill)
