...
def _wizard_summary_lines(
    *,
    ...
    if will_install:
        lines.append(f"- Install ChunkHound now: yes (source={install_source})")
    else:
        lines.append("- Install ChunkHound now: no")
    return lines
...
def run_chunkhound_setup_wizard(
    *,
    ...
    while True:
        ...
        force_install = bool(getattr(install_args, "force", False))
        skip_install = bool(getattr(install_args, "skip_install", False))
        existing_chunkhound = shutil.which("chunkhound")
        chunkhound_missing = existing_chunkhound is None or force_install
        run_install = False
        install_source: str | None = None
        explicit_source = bool(install_args is not None and hasattr(install_args, "chunkhound_source"))
        if skip_install:
            if existing_chunkhound is None:
                raise ReviewflowError(
                    "ChunkHound is not available on PATH. Remove --skip-install or install ChunkHound manually."
                )
        elif explicit_source or force_install:
            run_install = True
            install_source = str(getattr(install_args, "chunkhound_source", "release") or "release").strip()
        elif chunkhound_missing:
            install_raw = _read_wizard_line(
                prompt="ChunkHound is not available on PATH. Install now? [Y/n]: ",
                stdin=in_stream,
                stderr=err_stream,
            )
        ...
        if existing_chunkhound is None and not run_install:
            raise ReviewflowError(
                "ChunkHound CLI is still not available on PATH. "
                f"Rerun {PRIMARY_CLI_COMMAND} setup or install ChunkHound manually before retrying the original command."
            )
        if run_install:
            assert install_source is not None
            _reviewflow()._install_chunkhound(chunkhound_source=install_source, stderr=err_stream)
        final_selection = resolve_local_agent_selection(
            base_codex_config_path=runtime.codex_base_config_path,
            reviewflow_config_path=runtime.config_path,
            config_enabled=runtime.config_enabled,
            cli_preset=getattr(install_args, "llm_preset", None) if install_args is not None else None,
            env=os.environ,
        )
...
def setup_flow(
    ...
    *,
    ...
    if skip_install:
        if existing_chunkhound is None:
            raise ReviewflowError(
                "ChunkHound is not available on PATH. Remove --skip-install or install ChunkHound manually before retrying."
            )
    elif existing_chunkhound and not force and not explicit_source:
        print(f"Reusing existing ChunkHound on PATH: {existing_chunkhound}", file=err_stream)
    else:
        chunkhound_source = str(getattr(args, "chunkhound_source", "release") or "release").strip()
        _reviewflow()._install_chunkhound(chunkhound_source=chunkhound_source, stderr=err_stream)
