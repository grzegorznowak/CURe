...
Lifecycle diagnostics MUST exclude credentials, raw environments, daemon authentication material, and unredacted sensitive stderr.

#### Scenario: Failure data contains secrets
- **Given** secret-bearing environment and subprocess output
- **When** CURe reports lifecycle failure
...
### Requirement: Installed wheel executes the lifecycle

The installed CURe wheel MUST complete an eligible keeper/helper lifecycle through its installed `cure` executable.

#### Scenario: Isolated wheel lifecycle runs
- **Given** an isolated wheel and fake external executables
- **When** an eligible lifecycle runs
- **Then** the installed executable completes keeper acquisition, helper attachment, and cleanup
