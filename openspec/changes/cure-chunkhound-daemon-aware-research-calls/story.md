...
- CURRENT CORRECTION PASS — retained one owned lease/generation while waiting only on exact typed `status="initializing"` plus `query_ready=false`; each probe rechecks liveness/generation, authoritative top-level daemon `status` determines degraded/terminal state while backend-dependent `scan_progress` remains opaque, and no search/helper/model dispatch occurs before ready. Exactly one retry remains only for typed `PreNativeSpawnLeaseOpenError` before native spawn and before any helper/model dispatch.
- CURRENT CORRECTION PASS — full pytest: `937 passed, 4 skipped, 122 subtests`.
- CURRENT CORRECTION LIVE PASS — enabled TAP-05 exact four-case non-empty/zero-chunk × absent/existing-real-parent matrix: `4 passed in 76.85s` (latest), using the production readiness primitive with a 600-second deadline, 0.5-second polling, and a separate 60-second witness-search timeout.
- CURRENT CORRECTION ARTIFACT PASS — isolated sdist/wheel build, Twine checks, and external-cwd installed-wheel lifecycle smoke, including delayed-ready and timeout cases without claiming installed CLI `_pr_flow_impl` execution.
- CURRENT CORRECTION STATIC PASS — `ruff check .`; configured mypy (9 files); scoped lifecycle mypy (3 files); strict OpenSpec change validation; detect-secrets; unstaged and cached `git diff --check`.
- CURRENT CORRECTION REVIEW — independent final review: GO.
...
| TAP-02 | Keeper process integration | Ordering, retention, bounded native-readiness waiting, routing gates, strict lossless receipt/status/search boundaries in normal/quiet/no-stream modes, witness/zero-chunk branches, generation-bound filter authority, loss, close, and release (A1–A3, A7–A15, A22) | Same focused module/public façade | Sealed complete final-index capture from visible or silent pump transport plus real stdio fake MCP process and OS lifecycle | Exact typed transient `status="initializing"` plus `query_ready=false` may repeat until exact ready/query-ready on one retained lease and generation, with liveness/generation rechecked per probe and no search before ready; authoritative top-level daemon `status` determines degraded/terminal state while backend-dependent `scan_progress` remains opaque; bounded timeout, malformed, transport, liveness, or generation faults fail closed and close once with no helper/model/search dispatch; all prior receipt, identity, witness, release, and redaction assertions remain | Executable fake ChunkHound parameterized with deterministic initializing→ready, timeout, degraded, malformed, transport-loss, and generation-change status sequences in addition to the existing display, receipt, witness, generation-ledger, crash/hang, and secret modes | Focused public node IDs and full pytest | Linux-only skip for unsupported process semantics; use typed exact transient classification and a deterministic clock, never message matching, pre-normalized dictionaries, bounded tails, mocks, or an in-memory non-stream authority | Strict external shapes, retained-generation readiness, and generation-bound keeper adjudication stay together; transport/display mechanics remain TAP-01 and public route orchestration TAP-03 |
| TAP-03 | Fresh PR orchestration/process ownership | Standard/big/multipass, normal/quiet/explicit-no-stream routing, retained-lease readiness, concurrency, race-closed provider/helper descendant cleanup, faults, exact creation-only source boundary and generation-bound effective-filter gate (A1, A3, A4, A7–A14, A22) | Same focused module/public façade | Actual `_pr_flow_impl` through final top-up, materialized config/effective-filter startup gate, one keeper open, the production readiness primitive, `cure_output.py`/`run.py` capture transport, `cure_llm.py`, tagged registry, direct helper preflight, worker pool, and cleanup stack | Exact initializing/false waits on one lease/generation with one pre-spawn validation and no close/reopen or daemon-log precondition rerun after native creation; no helper/model/search occurs before ready and one witness search occurs after ready; timeout, degraded, malformed, transport, liveness, or generation failure closes once and leaves no owned residue or dispatch; all prior display, source-boundary, exclusion, generation, ordering, overlap, publication-race, cleanup, and no-replay assertions remain | Temp git repo with delayed-readiness fake ChunkHound and deterministic clock plus absent-parent, existing-real-parent/log-absent, pre-existing-log, invalid-parent, effective-filter/runtime identity, generation-ledger, provider/helper descendant, publication-barrier, and untagged-sentinel fixtures | Focused public normal/quiet/no-stream flow node IDs and full pytest | Linux-only process-group tests; use exact public CLI args, canonical manifests, status barriers, and tagged spawn/publication seams rather than sleeps or broad scheduler mocks | Canonical orchestration owns the one-open/no-close-reopen/no-precondition-rerun and zero-pre-ready-dispatch ledger; TAP-02 owns exhaustive status-shape sequences |
...
| TAP-05 | Real ChunkHound | Installed clean-start reuse, production native-readiness waiting, expected-session continuity, exact daemon-log exclusion, source-boundary creation, and release (A2–A5, A11, A22; not A13) | `test_daemon_aware_chunkhound_live.py` | Installed ChunkHound in four disposable cases: non-empty and zero-chunk, each with absent and existing-real-parent/log-absent state; no seeded or manipulated pre-existing native generation; readiness uses the same production primitive as `_pr_flow_impl`, not a test-only poll helper | Immediate pre-open and pre-spawn absence; exactly one pre-spawn validation/open and one newly lease-owned generation; the 600-second/0.5-second production wait retains that lease/generation without close/reopen or pre-ready ordinary-client search, then preserves continuity through witness/zero adjudication, separate 60-second search, ordinary-client concurrency, pre-close, and final release; exact exclusion and source-boundary assertions remain | Four disposable tiny repos/configs/DBs crossing non-empty/zero × absent/existing parent; unique marker, preserved sibling fixtures, no pre-existing generation seeding/manipulation, and no LLM | Enabled opt-in four-case live command | Explicit prerequisite failure/skip remains valid in ordinary runs; completion is backed by the latest enabled production-route `4 passed in 76.85s`, while deterministic TAP-02/TAP-03 transition and fault proof remains mandatory | Real clean-start capability, asynchronous readiness, continuity, exclusion, and boundary proof supplements deterministic faults; it neither proves A13 nor owns seeded/pre-existing/unattested rejection |
...
| TAP-07 | Packaging/release | Installed lifecycle primitive, delayed native readiness, checkout isolation, and separately retained success/failure/Ctrl-C publication-race descendant residue cleanup evidence (A23–A25; packaging slices of A5, A13, A16–A21) | Wheel smoke and release workflow test | Built wheel outside checkout with clean import/XDG state; the readiness slice invokes the installed production lifecycle API directly with a fake MCP status sequence and does not execute installed CLI `_pr_flow_impl` | Lifecycle-only smoke accepts exact initializing/false→ready/true on one lease/generation with no close/reopen or pre-ready search; timeout fails closed, closes once, and leaves no keeper process or DB lock. TAP-03 separately owns zero pre-ready helper/model dispatch; existing installed entrypoint/import, PGID, interrupt, TERM/KILL, and cleanup assertions remain separate evidence | Disposable build/smoke venvs, fake executable with delayed-ready and never-ready modes, cooperative/TERM-ignoring descendants, and deterministic readiness/publication control channels | Isolated build/Twine/installed-wheel smoke and publish workflow | Linux release workflow remains authoritative; exhaustive status faults stay in TAP-02/TAP-03 | Installed-wheel delayed-readiness and timeout proof guards packaged lifecycle/API divergence without overclaiming installed CLI orchestration |

### Acceptance Proof Matrix

| Acceptance ID | Proof Maturity | Proof Method | Reviewer Action | Expected Evidence | Relevant Surfaces | Open Detail |
|---|---|---|---|---|---|---|
| A1 | final | TAP-02, TAP-03 | Run keeper/flow ordering nodes including delayed-ready orientation-enabled route | `final-index/receipt-ready → retained keeper native-health/expected-session-ready → helper-preflight → orientation-or-first-review-model` with zero pre-ready search/helper/model events | `cure.py`, lifecycle module | Closed by current deterministic correction proof |
| A2 | final | TAP-02, TAP-05 | Run fake retained-readiness/gap proof and enabled live canary | One sanitized owned generation across readiness and sequential/concurrent clients with no close/reopen | Lease, helper, daemon observation | Closed by deterministic proof and latest enabled TAP-05 `4 passed in 76.85s` |
| A3 | final | TAP-02, TAP-03 | Run retained-readiness and phase-gap cases | One lease ID spans initialization and all applicable phases; only typed pre-native-spawn failure may retry before dispatch | `_pr_flow_impl` | Closed by current deterministic route proof |
...
| A10 | final | TAP-02, TAP-03 | Run missing-tool, top-level degraded-status, timeout/malformed/transport/liveness/generation, receipt/witness, and identity fault nodes | Zero helper/model dispatch, including orientation; one cleanup; only typed `PreNativeSpawnLeaseOpenError` retries before native spawn | Bootstrap/native-status/expected-session/identity/filter-generation gates | Closed by current deterministic correction proof; backend-dependent `scan_progress` remains opaque |
...
| A11 — non-empty witness | final | TAP-02, TAP-05 | Run deterministic non-empty fake and live witness nodes | No search before ready; then path-constrained native `search` with its separate 60-second timeout returns the selected Git-tracked path and literal | Witness selector, keeper session, session DB | Closed by current deterministic proof and latest enabled TAP-05 |
...
| A22 | final | TAP-02, TAP-03, TAP-05, TAP-06 | Use TAP-02/TAP-03 for seeded/pre-existing/unattested rejection and retained-generation faults; TAP-06 for static/config/manifest proof; TAP-05 for four clean-start non-empty/zero × absent/existing-parent cases | Deterministic proof rejects every pre-existing/unattested generation and retains only the newly opened CURe-owned generation through readiness. Live proof observes one pre-spawn validation/open, no close/reopen or pre-ready search, continuity through clients/pre-close, release, exact effective exclusion, and exact source boundary in all four cases | Config materializer/effective-filter generation gate, deterministic generation ledgers, keeper/index/cleanup, live native tools and DB/corpus evidence | Closed — latest enabled production-readiness proof passed `4 passed in 76.85s`; default skips and historical runs are not current completion evidence |
| A23 | final | TAP-07 | Run isolated build/Twine/wheel smoke and release owner | Installed production lifecycle API completes delayed-ready and timeout cases; installed `cure --help` verifies the entrypoint separately | Wheel and smoke harness | Final packaging evidence; no claim that smoke executes installed CLI `_pr_flow_impl` |
...
| Startup | Exact typed transient `status="initializing"` plus `query_ready=false` before dispatch | Retain one opened owned lease/generation for the 600-second readiness budget at 0.5-second polling; do not close/reopen or rerun pre-spawn checks | behavior | TAP-02 fake MCP process; TAP-03 real-flow ledger; TAP-05 production-primitive live canary | One lease/generation, no pre-ready search/helper/model, and bounded timeout cleanup | Exactly one retry is available only for typed `PreNativeSpawnLeaseOpenError` before native spawn and before any helper/model dispatch |
| Capability/health | Missing tool/payload field, exact initializing/false, ready/true, or degraded/malformed status | Wait only for exact initializing/false; proceed only on exact ready/true; authoritative top-level daemon `status` determines degraded/terminal state while backend-dependent `scan_progress` remains opaque | behavior | TAP-02 fake native MCP; TAP-05 live status canary | Exact native payload sequences and real asynchronous readiness are observed | No warning-only fallback, broad message matching, or backend-specific nested-progress interpretation |
...
| Native status | Raw MCP `daemon_status` result | Exact typed `initializing`/false is the only waitable state; exact ready/true proceeds; authoritative top-level daemon `status` determines degradation/terminality while backend-dependent dictionary `scan_progress` remains opaque | Initializing/false→ready/true, never-ready timeout, non-object, missing field, wrong type, any other status/query-ready combination, top-level degraded state, generation/liveness loss, transport fault, or error envelope | TAP-02 deterministic sequences; TAP-03 route ledger; TAP-05 production-primitive live canary; TAP-07 installed delayed/timeout smoke | Recheck one lease/generation per probe; dispatch no search/helper/model before ready; terminal faults close once with bounded redacted diagnostics |
...
- Native `daemon_status` and `search` results are strict external shapes: error envelopes, non-objects, missing/wrong-type required fields, authoritative top-level degraded/terminal `status`, malformed result collections/hits, or wrong/empty witnesses fail before model and never authorize the zero branch; backend-dependent dictionary `scan_progress` remains opaque.
...
- Allow exactly one startup retry only for typed `PreNativeSpawnLeaseOpenError` before native spawn and before any helper/model dispatch; do not retry or close/reopen the retained transiently initializing generation. Each attempt independently probes the exact config/runtime identity, rejects every pre-existing/unattested same-root generation, and accepts only its newly opened CURe-owned generation.
