...
- [x] Add `$PRIOR_CONTEXT` to the 5 built-in templates with usage documentation: normal singlepass, big singlepass, multipass plan/step/synth
- [x] Inject `build_pr_context()` in `cure.py::_pr_flow_impl` after `compute_pr_stats`, passing effective `head_sha` (`review_head_sha` if it exists, otherwise PR API `head_sha`)
- [x] Always pass `PRIOR_CONTEXT` (brief or `""`) in `extra_vars` for normal singlepass, big singlepass, and multipass plan/step/synth
